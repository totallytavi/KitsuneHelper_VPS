...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addChannelOption((option) => {
      return option.setName('channel').setDescription('The channel to nuke').setRequired(true);
    })
    .addStringOption((option) => {
      return option.setName('reason').setDescription('The reason for the nuke').setRequired(false);
    }),
  /**
  ...
  run: async (client, interaction, options) => {
    ...
    if (!interaction.guild.members.me.permissionsIn(channel).has('MANAGE_CHANNEL')) {
      return interactionEmbed(3, '[ERR-BPRM]', `Missing: \`Manage Channel\` > ${channel}`, interaction, client, true);
    }
    if (channel.parent && !interaction.guild.members.me.permissionsIn(channel.parent).has('MANAGE_CHANNEL')) {
      return interactionEmbed(
        3,
        '[ERR-BPRM]',
        `Missing: \`Manage Channel\` > ${channel.parent}`,
        interaction,
        client,
        true
      );
            ...
            channel.send({
              embeds: [
                new MessageEmbed()
                  ...
                  .setDescription(
                    `\`${channel.name}\` (\`${channel.id}\`) was nuked by ${interaction.member} (\`${interaction.user.id}\`) for \`${reason}\``
                  )
                  .setColor('#ff0000')
                  .setTimestamp()
              ]
