...
module.exports = {
    ...
    .addSubcommand((command) => {
      return command
        ...
        .addChannelOption((option) => {
          return option.setName('channel').setDescription('The channel to delete').setRequired(true);
        })
        .addStringOption((option) => {
          return option.setName('reason').setDescription('Reason for deleting the channel').setRequired(false);
        });
    })
    .addSubcommand((command) => {
      return command
        ...
        .addRoleOption((option) => {
          return option.setName('role').setDescription('The role to delete').setRequired(true);
        })
        .addStringOption((option) => {
          return option.setName('reason').setDescription('Reason for deleting the role').setRequired(false);
        });
    }),
  /**
  ...
  run: async (client, interaction, options) => {
    ...
    const confirmation = await awaitButtons(
      ...
      15,
      [
        new MessageButton({ customId: 'yes', label: 'Yes, I want to continue', style: 'DANGER' }),
        new MessageButton({ customId: 'no', label: "No, I don't want to continue", style: 'PRIMARY' })
      ],
      'Are you sure you want to continue with deletion? This is irreversible!',
      true
    );
      ...
      if (subcommand === 'channel') {
        if (!interaction.member.permissionsIn(option).has('MANAGE_CHANNELS'))
          return interactionEmbed(
            3,
            '[ERR-UPRM]',
            `Missing: \`Manage Channels\` > ${option}`,
            interaction,
            client,
            true
          );
        if (!interaction.guild.members.me.permissionsIn(option).has('MANAGE_CHANNELS'))
          return interactionEmbed(
            3,
            '[ERR-BRPM]',
            `Missing: \`Manage Channels\` > ${option}`,
        ...
        return interactionEmbed(
          1,
          `Successfully removed channel: ${option.name} (${option.id})`,
          '',
          interaction,
          client,
          true
        ...
        return interactionEmbed(
          1,
          `Successfully removed role: ${option.name} (${option.id})`,
          '',
          interaction,
          client,
          true
