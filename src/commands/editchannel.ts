...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((command) => {
      return command
        ...
        .addChannelOption((option) => {
          return option.setName('channel').setDescription('Channel to be updated').setRequired(true);
        })
        .addStringOption((option) => {
          return option.setName('name').setDescription('New name').setRequired(true);
        })
        .addStringOption((option) => {
          return option.setName('reason').setDescription('Reason for editing').setRequired(false);
        });
    })
    .addSubcommand((command) => {
      return command
        ...
        .addStringOption((option) => {
          return option.setName('topic').setDescription('New topic').setRequired(true);
        })
        .addStringOption((option) => {
          return option.setName('reason').setDescription('Reason for editing').setRequired(false);
        });
    ...
    .addSubcommand((command) => {
      return command
        ...
        .addBooleanOption((option) => {
          return option.setName('nsfw').setDescription('New NSFW flag').setRequired(true);
        })
        .addStringOption((option) => {
          return option.setName('reason').setDescription('Reason for the change').setRequired(false);
        });
    })
    .addSubcommand((command) => {
      return command
        ...
        .addNumberOption((option) => {
          return option.setName('seconds').setDescription('Slowmode in seconds').setRequired(true);
        })
        .addStringOption((option) => {
          return option.setName('reason').setDescription('Reason for slowmode').setRequired(false);
        });
    })
    .addSubcommand((command) => {
      return command
        ...
        .addNumberOption((option) => {
          return option.setName('limit').setDescription('New user limit (0-100)').setRequired(true);
        })
        .addStringOption((option) => {
          return option.setName('reason').setDescription('Reason for the change').setRequired(false);
        });
    ...
    .addSubcommand((command) => {
      return command
        ...
        .addStringOption((option) => {
          return option.setName('reason').setDescription('Reason for changing the bitrate').setRequired(false);
        });
    })
    .addSubcommand((command) => {
      return command
  ...
  run: async (client, interaction, options) => {
    ...
    const option =
      ...
      options.getBoolean('nsfw') ??
      options.getNumber('seconds') ??
      options.getString('bitrate') ??
      options.getNumber('limit');

    // If we cannot view the channel, stop
    if (!interaction.guild.members.me.permissionsIn(channel).has('VIEW_CHANNEL'))
    ...
    if (!interaction.guild.members.me.permissionsIn(channel).has('MANAGE_CHANNELS'))
    ...
    if (options._hoistedOptions[1].name === 'name') {
      channel
        ...
        .then((newChannel) =>
          interactionEmbed(
            1,
            `Set ${channel}'s name was set to \`${newChannel.name}\` for \`${reason}\``,
            '',
            interaction,
            client,
            false
      ...
      if (!channel.isText())
        return interactionEmbed(
          3,
          '[ERR-ARGS]',
          'Arg: channel :-: Expected TextBasedChannel, got Category/VoiceBasedChannel',
          interaction,
          client,
          true
        );
      channel
        .setTopic(option, reason)
        .then((newChannel) =>
          interactionEmbed(
            1,
            `Set ${channel}'s topic was set to \`${newChannel.topic}\` for \`${reason}\``,
            '',
            interaction,
            client,
            false
      ...
      channel
        ...
        .then((newChannel) =>
          interactionEmbed(
            1,
            `Set ${channel}'s NSFW flag to \`${newChannel.nsfw}\` for \`${reason}\``,
            '',
            interaction,
            client,
            false
      ...
      channel
        ...
        .then((newChannel) =>
          interactionEmbed(
            1,
            `Set ${channel}'s slowmode to \`${newChannel.rateLimitPerUser}\` for \`${reason}\``,
            '',
            interaction,
            client,
            false
      ...
      if (!channel.isVoice())
        return interactionEmbed(
          3,
          '[ERR-ARGS]',
          'Arg: channel :-: Expected VoiceBasedChannel, got TextBasedChannel',
          interaction,
          client,
          true
        );
      channel
        .setBitrate(option, reason)
        .then((newChannel) =>
          interactionEmbed(
            1,
            `Set ${channel}'s bitrate to \`${newChannel.bitrate}\` for \`${reason}\``,
            '',
            interaction,
            client,
            false
      ...
      channel
        ...
        .then((newChannel) =>
          interactionEmbed(
            1,
            `Set ${channel}'s user limit to \`${newChannel.userLimit}\` for \`${reason}\``,
            '',
            interaction,
            client,
            false
      ...
      channel
        ...
        .then((newChannel) =>
          interactionEmbed(
            1,
            `Set ${newChannel}'s permissions to the category's permissions`,
            '',
            interaction,
            client,
            false
