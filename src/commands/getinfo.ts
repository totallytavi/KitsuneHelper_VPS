 ...
 */
export async function run(_client: KitsuneClient, interaction: CommandInteraction<'cached'>, options: CommandInteractionOptionResolver) {
  ...
  if (subcommand === "channel") {
    const option = options.getChannel("channel", true) as GuildChannel;

    const embed = new EmbedBuilder()
      .setTitle("Channel Information")
      .addFields(
        { name: "Created At", value: `<t:${Math.floor(option.createdAt.getTime() / 1000)}:F>`, inline: true },
        { name: "Name", value: option.name, inline: true },
        { name: "ID", value: option.id, inline: true },
        { name: "Position", value: option.position.toString(), inline: true },
        { name: "Type", value: String(option.type), inline: true }
      )
      .setTimestamp(new Date());
    
    if ('topic' in option) {
      embed.setDescription(option.topic as string);
    } else {
    ...
    const option= options.getMember("magician") as GuildMember;
    const option2 = options.getChannel("channel");
