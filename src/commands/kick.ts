...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) => {
      return option.setName('user').setDescription('The magician to be removed').setRequired(true);
    })
    .addStringOption((option) => {
      return option.setName('reason').setDescription('The reason for removing the magician').setRequired(false);
    }),
  /**
  ...
  run: async (client, interaction, options) => {
    ...
    if (!interaction.member.permissions.has('KICK_MEMBERS'))
      return interactionEmbed(3, '[ERR-UPRM]', 'Missing: `Kick Members`', interaction, client, true);
    ...
    if (interaction.user.id === member.user.id)
      return interactionEmbed(
        3,
        '[ERR-ARGS]',
        'Arg: member :-: Expected other user, got same user',
        interaction,
        client,
        true
      );
    if (interaction.member.roles.highest.rawPosition <= member.roles.highest.rawPosition)
      return interactionEmbed(
        3,
        '[ERR-ARGS]',
        'Arg: member :-: Expected user lower than executor, got user at or above executor',
        interaction,
        client,
        true
      );
    if (interaction.guild.members.me.roles.highest.rawPosition <= member.roles.highest.rawPosition)
      return interactionEmbed(
        3,
        '[ERR-ARGS]',
        'Arg: member :-: Expected user lower than bot, got user at or above bot',
        interaction,
        client,
        true
      );
    ...
    const confirmation = await awaitButtons(
      ...
      15,
      [
        new MessageButton({ customId: 'yes', label: 'Yes, I want to kick the magician', style: 'DANGER' }),
        new MessageButton({ customId: 'no', label: 'No, I do not want to kick the magician', style: 'SUCCESS' })
      ],
      'Are you sure you want to kick this magician?',
      true
    );
    if (confirmation.customId === 'yes') {
      await member.kick(`${reason} (Moderator ID: ${interaction.user.id})`);
      const result = await client.connection
        .execute('INSERT INTO Kicks(kickId, guildId, userId, modId, reason) VALUES(?, ?, ?, ?, ?)', [
