...
(async () => {
    ...
    try {
        ...
        await rest.put(Routes.applicationCommands(bot["applicationId"]), { body: slashCommands });
        const then = Date.now();
...
client.on("clientReady", async () => {
    ...
    setInterval(async () => {
        ...
        for (const ban of validBans) {
            if (!guildBans[ban.guildId])
                guildBans[ban.guildId] = [];
    ...
    if (interaction.isCommand() && 'options' in interaction) {
        ...
        await interaction.fetchReply()
            ...
            if (m.content === "" && m.embeds.length === 0)
                interactionEmbed(3, "[ERR-UNK]", "The command timed out and failed to reply in 10 seconds", interaction, client, false);
        });
    }
});
