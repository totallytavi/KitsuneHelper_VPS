'use strict';
import { ActivityType, Client, Collection, IntentsBitField, InteractionType } from "discord.js";
...
const client = new Client({
    intents: [IntentsBitField.Flags.Guilds, IntentsBitField.Flags.GuildBans, IntentsBitField.Flags.GuildMembers, IntentsBitField.Flags.GuildInvites]
});
...
client.on("ready", async () => {
    toConsole(`[READY] Logged in as ${client.user.tag} (${client.user.id}) at <t:${Math.floor(Date.now() / 1000)}:T>. Client ${ready ? "can" : "**cannot**"} receive commands!`, new Error(), client);
    client.guilds.cache.each(g => g.members.fetch());
    client.user.setActivity(`${client.users.cache.size} magicians across ${client.guilds.cache.size} forests`, { type: ActivityType.Listening });
    ...
    setInterval(async () => {
        ...
        client.user.setActivity(`${client.users.cache.size} magicians across ${client.guilds.cache.size} forests`, { type: ActivityType.Listening });
        if (ready) {
            const bans = await client.models.ModAction.findAll({ where: { type: "Ban", duration: { [Op.lte]: Date.now() } } });
            const p = [];
            // Fetch all servers' bans
            bans.forEach((b, index) => {
                // Get the guild
                const guild = client.guilds.cache.get(b.guild);
                if (guild) {
            ...
            bans.forEach(b => {
                const guild = client.guilds.cache.get(b.guild);
                if (guild) {
                    const ban = guild.bans.cache.get(b.user);
                    if (ban) {
                        // Unban
                        p.push(guild.members.unban(b.user, `Ban expired (Moderator ID: ${client.user.id})`));
                        // Set ban to inactive
                        p.push(client.models.ModAction.update({ active: false }, { where: { id: b.id } }));
                    }
                }
            });
            // Settle all Promises
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (!interaction.inGuild())
        return interactionEmbed(2, "[WARN-NODM]", "", interaction, client, [true, 10]);
        ...
        case InteractionType.ModalSubmit: {
            ...
            if (modal) {
                ...
                interaction.fetchReply()
                    .then(m => {
                    if (m.content === "" && m.embeds.length === 0)
                        interactionEmbed(3, "[ERR-UNK]", "The modal timed out and failed to reply in 10 seconds", interaction, client, [true, 15]);
                });
            }
            break;
        }
...
process.on("unhandledRejection", async (promise) => {
    ...
    if (!suppressChannel)
        return console.error(`An [unhandledRejection] has occurred.\n\n> ${promise}`);
