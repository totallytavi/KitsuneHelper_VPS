...
export async function toConsole(message, source, client) {
    if (!message)
        return new SyntaxError("message is undefined");
    if (!source)
        return new SyntaxError("source is undefined");
    if (!client)
        return new SyntaxError("client is undefined");
    const channel = await client.channels.fetch(bot["errorChannel"]);
    if (!channel)
        return console.warn("[WARN] Error channel note found but data was sent to the console\n\n" + message + "\n> Source: " + source);
    if (!channel.isTextBased())
        return console.warn("[WARN] Error channel is not a text channel but data was sent to the console\n\n" + message + "\n> Source: " + source);
    channel.send({
...
export async function interactionEmbed(type, content, expected, interaction, client, ephemeral) {
    if (!type || !content || !interaction || !client || ephemeral === undefined)
        return new SyntaxError(`One or more arguments for interactionEmbed have received an invalid value\n> type: ${type}\n> content: ${content}\n> interaction: ${interaction}\n> client: ${client}\n> ephemeral: ${ephemeral}`);
...
export async function awaitButtons(interaction, time, buttons, content, remove) {
    if (!interaction || !time || !buttons || remove === null)
        throw new SyntaxError(`One of the following values is not fulfilled:\n> interaction: ${interaction}\n> time: ${time}\n> buttons: ${buttons}\n> remove: ${remove}`);
    ...
    setTimeout(async () => {
        // Clear buttons
        if (remove && res != null)
            message.edit({ content: (await interaction.fetchReply()).content, components: [] });
    }, 5000);
...
export async function awaitMenu(interaction, time, values, options, content, remove) {
    // Step 0: Checks
    if (!interaction || !time || !values || !options || remove === null)
        throw new SyntaxError(`One of the following values is not fulfilled:\n> interaction: ${interaction}\n> time: ${time}\n> values: ${values}\n> options: ${options}\n> remove: ${remove}`);
    content = content ?? "Please select an option";
    // Step 1: Setup
    const filter = (i) => {
