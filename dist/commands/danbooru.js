    ...
    run: async (client, interaction, options) => {
        ...
        fetch(`https://danbooru.donmai.us/posts.json?tags=${tags}&${params.toString()}`, { headers: { "User-Agent": "KitsuneHelper/0 (+github.com/Coder-Tavi/KitsuneHelper_VPS)" }, timeout: 5000 })
            ...
            if (json.length === 0 || json.success === false)
                return interactionEmbed(3, "[ERR-EXPT]", json.length === 0 ? `\`${tags.split("+")[0]}\` doesn't exist on Danbooru` : `Something went wrong when requesting data from Danbooru. Please report this to the support server:\n>>> Success? ${json.success}\nMessage: ${json.message}\nResponse:\n${json}`, interaction, client, false);
