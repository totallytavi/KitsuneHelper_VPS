...
import { CategoryChannel, ButtonBuilder, ButtonStyle } from "discord.js";
...
export async function run(client, interaction, options) {
    ...
    if (!confirmation)
        return interaction.editReply({ content: "No response after 15 seconds, the spell has been cancelled! No magic was performed", components: [] });
    if (confirmation.customId === "yes") {
        if (subcommand === "channel") {
            const option = options.getChannel("channel", true);
            if (!interaction.member.permissionsIn(option.id).has("ManageChannels"))
                return interactionEmbed(3, "[ERR-UPRM]", `Missing: \`Manage Channels\` > ${option}`, interaction, client, true);
            if (!interaction.guild.members.me.permissionsIn(option.id).has("ManageChannels"))
            ...
            if (interaction.member.roles.highest.comparePositionTo(option) >= 0)
                return interactionEmbed(3, "[ERR-UPRM]", `You cannot delete a role that is higher than your own!`, interaction, client, true);
            if (interaction.guild.members.me.roles.highest.comparePositionTo(option) <= 0)
                return interactionEmbed(3, "[ERR-BRPM]", `I cannot delete a role that is higher than my own!`, interaction, client, true);
            await option.delete(`${reason} (Moderator ID: ${interaction.user.id})`);
