...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        return option
            ...
            .setRequired(false)
            .addChoices({ name: "none", value: 0 }, { name: "1day", value: 1 }, { name: "2days", value: 2 }, { name: "3days", value: 3 }, { name: "4days", value: 4 }, { name: "5days", value: 5 }, { name: "6days", value: 6 }, { name: "7days", value: 7 });
    })
        .addStringOption(option => {
        return option
            .setName("duration")
    ...
    run: async (client, interaction, options) => {
        ...
        if (typeof duration === "undefined")
            duration = new Date(0); // Not a duration
        ...
        if (member.member) {
            if (interaction.member.roles.highest.rawPosition <= member.member.roles.highest.rawPosition)
                return interactionEmbed(3, "[ERR-ARGS]", "That magician is equal to or higher than you. Make sure your highest role is above theirs", interaction, client, true);
            if (interaction.guild.me.roles.highest.rawPosition <= member.member.roles.highest.rawPosition)
                return interactionEmbed(3, "[ERR-ARGS]", "That magician is equal to or higher than me. Make sure my highest role is above theirs", interaction, client, true);
        }
