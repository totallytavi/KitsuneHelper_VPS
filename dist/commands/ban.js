...
export const data = new SlashCommandBuilder()
    ...
    return option
        ...
        .setRequired(false)
        .addChoices({ name: "None", value: 0 }, { name: "1 day", value: 1 }, { name: "2 days", value: 2 }, { name: "3 days", value: 3 }, { name: "4 days", value: 4 }, { name: "5 days", value: 5 }, { name: "6 days", value: 6 }, { name: "7 days", value: 7 });
})
    .addStringOption(option => {
    return option
        .setName("duration")
...
export async function run(client, interaction, options) {
    ...
    if (!interaction.guild.members.me.permissions.has("BanMembers"))
        return interactionEmbed(3, "[ERR-BPRM]", "Missing: `Ban Members`", interaction, client, true);
    if (interaction.user.id === member.id)
        return interactionEmbed(3, "[ERR-ARGS]", "Arg: member :-: Cannot ban yourself", interaction, client, true);
    const guildMember = await interaction.guild.members.fetch(member.id);
    if (guildMember) {
        if (guildMember.roles.highest.position >= interaction.member.roles.highest.position)
            return interactionEmbed(3, "[ERR-ARGS]", "Arg: member :-: Target is higher than or equal to your highest role", interaction, client, true);
        if (guildMember.roles.highest.position >= interaction.guild.members.me.roles.highest.position)
            return interactionEmbed(3, "[ERR-ARGS]", "Arg: member :-: User is equal to or higher than the bot's highest role", interaction, client, true);
    }
    const buttons = [
        new ButtonBuilder()
            .setCustomId("yes")
