...
export const data = new SlashCommandBuilder()
    ...
    return option
        ...
        .setRequired(false)
        .addChoices({ name: "1", value: 1 }, { name: "2", value: 2 }, { name: "3", value: 3 }, { name: "4", value: 4 }, { name: "5", value: 5 }, { name: "6", value: 6 }, { name: "7", value: 7 }, { name: "8", value: 8 }, { name: "9", value: 9 }, { name: "10", value: 10 });
})
...
export async function run(client, interaction, options) {
    if (!interaction.channel.isTextBased())
        return interactionEmbed(2, "[ERR-ARGS]", "This command can only be used in text channels!", interaction, client, true);
    ...
    fetch(`https://${image_board["domain"]}/posts.json?tags=${tags}&${params.toString()}`, { headers: { "User-Agent": "KitsuneHelper/0 (+github.com/totallytavi/KitsuneHelper_VPS)" }, signal: abort.signal })
        ...
        if (json.length === 0 || json.success === false)
            return interactionEmbed(3, "[ERR-EXPT]", json.length === 0 ? `\`${tags.split("+")[0]}\` doesn't exist on ${image_board["name"]}` : `Something went wrong when requesting data from ${image_board["name"]}. Please report this to the support server:\n>>> Success? ${json.success}\nMessage: ${json.message}\nResponse:\n${json}`, interaction, client, false);
        for (const post of json) {
            ...
            passed++;
            if (skipped + passed === json.length)
                interaction.editReply({ content: `Skipped ${skipped} images` });
        }
    })
        .catch(e => {
