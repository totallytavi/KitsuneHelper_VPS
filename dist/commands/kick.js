...
export async function run(client, interaction, options) {
    ...
    if (interaction.user.id === member.user.id)
        return interactionEmbed(3, "[ERR-ARGS]", "Arg: member :-: Expected other user, got same user", interaction, client, true);
    if (interaction.member.roles.highest.rawPosition <= member.roles.highest.rawPosition)
        return interactionEmbed(3, "[ERR-ARGS]", "Arg: member :-: Expected user lower than executor, got user at or above executor", interaction, client, true);
    if (interaction.guild.members.me.roles.highest.comparePositionTo(member.roles.highest) <= 0)
        return interactionEmbed(3, "[ERR-ARGS]", "Arg: member :-: Expected user lower than bot, got user at or above bot", interaction, client, true);
    ...
    if (!confirmation)
        return interaction.editReply({ content: ":x: No response after 15 seconds, banishment cancelled, the magician remains in your forest!", components: [] });
