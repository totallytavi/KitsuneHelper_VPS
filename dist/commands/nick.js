import { ButtonBuilder, SlashCommandBuilder, ButtonStyle } from "discord.js";
...
export async function run(client, interaction, options) {
    ...
    if (member.user.id === client.user.id) {
        ...
        if (!interaction.member.permissions.has("ChangeNickname"))
    ...
    if (!member.manageable && member != botMember)
        return interactionEmbed(3, "[ERR-BPRM]", "I cannot change nicknames of those higher or equal to me", interaction, client, true);
    // If they can't manage them, reject it.
    if (member.roles.highest.rawPosition >= interaction.member.roles.highest.rawPosition)
        return interactionEmbed(3, "[ERR-UPRM]", "You cannot change nicknames of those higher or equal to you", interaction, client, true);
    // If the nickname is too long, reject it.
    if (nickname && nickname.length > 32)
        return interactionEmbed(3, "[ERR-ARGS]", "Arg: new_nickname :-: Expected String.length < 32, got String.length > 32", interaction, client, true);
